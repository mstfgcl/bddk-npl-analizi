...
sektor_riski AS (SELECT 
sektör,
ROUND(SUM(takipteki_krediler*100.0)/SUM(toplam_nakdi_krediler),2) AS sektor_npl_orani
FROM tum_aylar
GROUP BY sektör)
...
t.sektor_npl_orani,
CASE 
   WHEN t.sektor_npl_orani &gt;r.uc_kat THEN 'Kritik Risk'
   WHEN t.sektor_npl_orani &gt; r.iki_kat THEN 'Yüksek Risk'
   WHEN t.sektor_npl_orani&gt; r.npl THEN 'Orta Risk'
   ELSE 'Düşük Risk'
   END AS risk_seviyesi
   
  FROM sektor_riski t, risk_kategorileri r
  GROUP BY t.sektör
  ORDER BY t.sektor_npl_orani DESC
