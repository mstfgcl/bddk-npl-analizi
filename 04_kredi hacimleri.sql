...
SELECT
sektör,
kredi_hacmi_MilyarTL,
CASE
     WHEN sektor_npl &gt; senelik_npl*3 THEN 'Kritik Risk'  
     WHEN sektor_npl &gt; senelik_npl*2 THEN ' Yuksek Risk' 
	 WHEN sektor_npl &gt; senelik_npl THEN ' Orta Risk'  
	  ELSE ' Düşük Risk'
END AS risk_seviyesi
FROM sektor_bazli_npl , yillik_npl 
ORDER BY
    CASE
	     WHEN sektor_npl &gt; senelik_npl*3 THEN  1
         WHEN sektor_npl &gt; senelik_npl*2 THEN  2
	     WHEN sektor_npl &gt; senelik_npl THEN  3
	     ELSE 4
	END )
	
SELECT
risk_seviyesi,
COUNT(r.sektör)AS sektor_sayisi,
ROUND(SUM(kredi_hacmi_MilyarTL),2) AS risk_bazli_kredi_hacmi_MilyarTL
FROM risk_kategorileri r
GROUP BY risk_seviyesi
ORDER BY risk_bazli_kredi_hacmi_MilyarTL DESC
